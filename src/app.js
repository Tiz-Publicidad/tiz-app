...
// ============================================================
// DRIVE OT — integración mínima y estable por JSONP
// ============================================================
const TIZ_DRIVE_OT_WEBHOOK = "https://script.google.com/macros/s/AKfycbx_Uy_ijUG38rht-m-Xp-y9Eou8WzoG4jepXi1GqJaHAknwQsQd-rQgYcQ1ucrAJPlK/exec";

async function syncToSheets(data) {
  if (!data || String(data.estado || '').trim().toLowerCase() !== 'aprobado') return {ok:true, skipped:true};
  if (!TIZ_DRIVE_OT_WEBHOOK || !TIZ_DRIVE_OT_WEBHOOK.includes('/exec')) throw new Error('Falta configurar la URL /exec de Apps Script');

  // Se envía un payload deliberadamente pequeño para evitar el problema histórico
  // de URLs JSONP demasiado largas. La carpeta OT no necesita toda la cotización.
  const compact = {
    action: 'ensureOtFolder',
    obra: {
      firestoreId: data.firestoreId || '',
      ot: String(data.ot || '').trim(),
  ...
  return await new Promise((resolve, reject) => {
    const callback = '__tizDriveCb_' + Date.now() + '_' + Math.random().toString(36).slice(2);
    const script = document.createElement('script');
    const timer = setTimeout(() => cleanup(new Error('Apps Script no respondió en 20 segundos')), 20000);
    ...
    window[callback] = result => {
      if (!result || result.ok !== true) cleanup(new Error(result?.error || 'Error desconocido de Apps Script'));
      else cleanup(null, result);
    };
    script.onerror = () => cleanup(new Error('No se pudo conectar con Apps Script'));
    const qs = new URLSearchParams({ callback, payload: JSON.stringify(compact), _t: String(Date.now()) });
    script.src = TIZ_DRIVE_OT_WEBHOOK + '?' + qs.toString();
...
// ============================================================
// NUEVA OBRA — reinicio definitivo del formulario
// ============================================================
