...
// ============================================================
// SYNC CON GOOGLE DRIVE / APPS SCRIPT — CORREGIDO
// ============================================================
// URL real de la implementación Web App de Apps Script.
// Usa JSONP por GET para evitar bloqueos CORS desde GitHub Pages.
...
function isAprobadoEstado(v) {
  return String(v || '').trim().toLowerCase() === 'aprobado';
}

function syncToSheetsJSONP(payload) {
  return new Promise((resolve) => {
    if (!DRIVE_SHEETS_WEBHOOK || String(DRIVE_SHEETS_WEBHOOK).includes('PEGAR_URL')) {
      resolve({ ok:false, error:'Falta configurar URL de Apps Script' });
      return;
    }

    const callback = 'tizDriveCb_' + Date.now() + '_' + Math.random().toString(36).slice(2);
    const script = document.createElement('script');
    const timer = setTimeout(() => {
      cleanup();
      resolve({ ok:false, error:'Timeout llamando Apps Script' });
    }, 25000);
    ...
    window[callback] = function(response) {
      cleanup();
      resolve(response || { ok:false, error:'Respuesta vacía de Apps Script' });
    };

    const url = DRIVE_SHEETS_WEBHOOK
    ...
    script.onerror = function() {
      cleanup();
      resolve({ ok:false, error:'No se pudo cargar Apps Script. Revisar publicación/acceso.' });
    };
...
async function syncToSheets(data) {
  const payload = {
    action: isAprobadoEstado(data.estado) ? 'obra_aprobada' : 'obra_guardada',
    obra: data,
    itemsCotizados: data.itemsCotizados || [],
    calculosAuxiliares: data.calculosAuxiliares || []
  };

  const parsed = await syncToSheetsJSONP(payload);

  if (isAprobadoEstado(data.estado)) {
    if (parsed && parsed.ok && (parsed.driveFolderUrl || parsed.otSheetUrl)) {
      return parsed;
    }
    showToast('No se creó carpeta Drive: ' + ((parsed && parsed.error) ? parsed.error : 'revisar Apps Script'));
