  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 1; i < cols.length - 1; i++)
        line(doc, cols[i], y, cols[i], y + 20, 0.4);
      headers.forEach((h, i) =>
        doc
          .font("Helvetica-Bold")
          .fontSize(6)
      ...
      const ivaPct =
        String(factura.alicuota) === "exento" ? 0 : n(factura.alicuota);
      items.slice(0, 40).forEach((it, i) => {
        const desc = safe(it.descripcion || it.desc || `Ítem ${i + 1}`),
          cant = n(it.cantidad) || 1,
          unit = n(it.unitario),
          sub = cant * unit,
          totalItem = sub * (1 + ivaPct / 100);
        ...
        doc.text(String(i + 1).padStart(4, "0"), cols[0] + 2, y + 3, {
          width: 34,
        });
        doc.text(desc, cols[1] + 2, y + 3, { width: cols[2] - cols[1] - 4 });
        doc.text(cant.toFixed(2).replace(".", ","), cols[2] + 2, y + 3, {
          width: cols[3] - cols[2] - 4,
          align: "right",
        });
        doc.text("unidades", cols[3] + 2, y + 3, {
          width: cols[4] - cols[3] - 4,
          align: "center",
        });
        doc.text(money(unit), cols[4] + 2, y + 3, {
          width: cols[5] - cols[4] - 4,
          align: "right",
        });
        doc.text("0,00", cols[5] + 2, y + 3, {
          width: cols[6] - cols[5] - 4,
          align: "right",
        });
        doc.text(money(sub), cols[6] + 2, y + 3, {
          width: cols[7] - cols[6] - 4,
          align: "right",
        });
        doc.text(
          String(factura.alicuota) === "exento" ? "Exento" : `${ivaPct}%`,
          cols[7] + 2,
          y + 3,
          { width: cols[8] - cols[7] - 4, align: "center" },
        );
        doc.text(money(totalItem), cols[8] + 2, y + 3, {
          width: cols[9] - cols[8] - 4,
          align: "right",
        });
        y += h;
      });
      ...
      rows.forEach((r, i) => {
        const yy = totalsTop + 12 + i * 12;
        doc
          .font(i === rows.length - 1 ? "Helvetica-Bold" : "Helvetica-Bold")
          .fontSize(i === rows.length - 1 ? 8 : 7.2)
          .text(r[0], tx, yy, { width: 90, align: "right" });
        doc
          .font("Helvetica-Bold")
          .text(`$ ${money(r[1])}`, valx, yy, { width: 72, align: "right" });
      });
      ...
      doc
        ...
        .fontSize(7)
        .text("Comprobante Autorizado", left + 105, foot + 52, { width: 170 });
      doc
        .font("Helvetica")
        .fontSize(5.5)
        .text(
          "Este comprobante fue autorizado electrónicamente por ARCA.",
          left + 105,
          foot + 66,
          { width: 210 },
        );
      doc
        .font("Helvetica-Bold")
        .fontSize(8)
        .text("CAE N°:", 390, foot + 18, { width: 70, align: "right" });
      doc
        .font("Helvetica")
        .text(String(factura.cae || ""), 465, foot + 18, { width: 90 });
      doc
        .font("Helvetica-Bold")
        .text("Fecha de Vto. de CAE:", 350, foot + 38, {
          width: 110,
          align: "right",
        });
      doc
        .font("Helvetica")
        .text(fmtDate(factura.caeVto), 465, foot + 38, { width: 90 });
      doc
        .font("Helvetica-Bold")
        .fontSize(7.5)
        .text("Pág. 1/1", 270, foot + 20, { width: 70, align: "center" });
      if (factura.asociado?.numeroCompleto)
        doc
          .font("Helvetica")
          .fontSize(6)
          .text(
            `Comprobante asociado: ${factura.asociado.tipo || ""} ${factura.asociado.numeroCompleto}`,
            350,
            foot + 62,
...
async function archivarFacturaPdfEnDrive({
  factura,
  obra,
  issuerCuit,
  accessToken,
  ...
  if (hit)
    return {
      ...
      fileName: hit.name,
      webViewLink:
        hit.webViewLink || `https://drive.google.com/file/d/${hit.id}/view`,
      folderId: FACTURAS_2026_FOLDER_ID,
      reused: true,
      authMode: accessToken ? "usuario" : "service-account",
    };
  ...
  const response = await drive.files.create({
    requestBody: {
      name: fileName,
      parents: [FACTURAS_2026_FOLDER_ID],
