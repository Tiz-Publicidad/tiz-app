...
function list(o){const a=Array.isArray(o.comprobantesArca)?[...o.comprobantesArca]:Array.isArray(o.facturasArca)?[...o.facturasArca]:[];if(o.facturaArca?.cae&&!a.some(x=>x.cae===o.facturaArca.cae))a.push(o.facturaArca);return a.filter(x=>x?.cae)}
...
const facturacionIntegralEmitirV83=onRequest({region:"us-central1",invoker:"public",secrets,timeoutSeconds:90},async(req,res)=>{
  ...
  let lock=null,op=null,authorized=false;
  try{
    const email=await operator(req);
    if(req.body?.confirmacion!=="EMITIR COMPROBANTE REAL PV 00009")throw Object.assign(new Error("Falta confirmación final"),{status:400});
    const obraId=String(req.body?.obraId||"").trim();
    const idKey=String(req.body?.idempotencyKey||"").replace(/[^A-Za-z0-9_-]/g,"").slice(0,80);
    const tipo=Number(req.body?.cbteTipo),cfg=validateType(tipo);
    ...
    const ref=db.collection("obras").doc(obraId),snap=await ref.get();
    ...
    const exento=String(req.body?.tratamientoIva||"gravado")==="exento";
    const ivaCfg=ivaConfig(req.body?.alicuota??21,exento),neto=round2(req.body?.neto);
    if(!(neto>0))throw Object.assign(new Error("Importe neto inválido"),{status:400});
    const items=(Array.isArray(req.body?.items)?req.body.items:[]).slice(0,60).map(x=>({descripcion:String(x.descripcion||x.desc||"").trim(),cantidad:Number(x.cantidad||1),unitario:Number(x.unitario||0)})).filter(x=>x.descripcion&&x.cantidad>0);
    if(!items.length)throw Object.assign(new Error("Debe haber al menos un ítem"),{status:400});
    const prev=list(obra),pres=round2(obra.finanzas?.total||obra.neto||obra.importe||obra.infoPresupuesto?.importe),fiscal=saldoFiscal(pres,prev),asoc=validateAssociated(tipo,req.body?.asociado||null);
    if(cfg.familia==="factura"&&neto-fiscal.saldo>0.01)throw Object.assign(new Error(`El neto supera el saldo por facturar (${fiscal.saldo.toFixed(2)})`),{status:400});
    ...
    if(old.exists&&old.data()?.status==="autorizada")return res.json({ok:true,...old.data().comprobante,repetida:true});
    ...
    await op.set({status:"procesando",obraId,tipo,neto,total,operador:email,iniciadoAt:admin.firestore.FieldValue.serverTimestamp()});
    const cr=await credentials();
    const lx=await call("FECompUltimoAutorizado",`<PtoVta>${PTO}</PtoVta><CbteTipo>${tipo}</CbteTipo>`,cr);
    const next=Number(tag(lx,"CbteNro")||0)+1;
    const asocXml=asoc?`<CbtesAsoc><CbteAsoc><Tipo>${asoc.cbteTipo}</Tipo><PtoVta>${asoc.ptoVta}</PtoVta><Nro>${asoc.cbteNro}</Nro>${cfg.fce?`<Cuit>${cuit}</Cuit>`:""}</CbteAsoc></CbtesAsoc>`:"";
    const ivaXml=exento?"":`<Iva><AlicIva><Id>${ivaCfg.id}</Id><BaseImp>${neto.toFixed(2)}</BaseImp><Importe>${iva.toFixed(2)}</Importe></AlicIva></Iva>`;
    const fceVto=cfg.fce?`<FchVtoPago>${vto.arca}</FchVtoPago>`:"";
    ...
    const xml=await call("FECAESolicitar",detail,cr),result=tag(xml,"Resultado"),cae=tag(xml,"CAE"),caeVto=tag(xml,"CAEFchVto"),msgs=[...xml.matchAll(/<(?:Msg|Obs)>([\s\S]*?)<\/(?:Msg|Obs)>/gi)].map(m=>dec(m[1].trim())).filter(Boolean);
    if(result!=="A"||!cae)throw Object.assign(new Error(msgs.join(" · ")||"ARCA rechazó el comprobante"),{status:422});
    authorized=true;
    const numero=`${String(PTO).padStart(5,"0")}-${String(next).padStart(8,"0")}`;
    const porcentaje=pres>0?round2(neto/pres*100):0;
    ...
    const all=[...prev,comp],nextFiscal=saldoFiscal(pres,all);
    comp.saldoPresupuestoNeto=nextFiscal.saldo;
    comp.porcentajeFacturadoAcumulado=pres>0?round2(nextFiscal.emitido/pres*100):0;
    comp.porcentajePendiente=pres>0?round2(nextFiscal.saldo/pres*100):0;
    const fin={...(obra.finanzas||{}),total:pres,diasPago:dias,facturadoNeto:nextFiscal.emitido,saldoFacturacionNeto:nextFiscal.saldo,porcentajeFacturado:comp.porcentajeFacturadoAcumulado,porcentajePendienteFacturar:comp.porcentajePendiente};
    await op.set({status:"autorizada",comprobante:comp,finalizadoAt:admin.firestore.FieldValue.serverTimestamp()},{merge:true});
    await ref.update({facturaArca:comp,comprobantesArca:all,facturasArca:all,finanzas:fin,nrfc:numero,ffc:hoy.iso,facturado:nextFiscal.saldo<=.01,facturaDrivePendiente:true,estadoGestionFactura:"Facturada - falta enviar",facturacionActualizadaAt:new Date().toISOString()});
    ...
    try{
      ...
      await ref.update({"facturaArca.driveFileId":drive.fileId,"facturaArca.driveFileName":drive.fileName,"facturaArca.driveWebViewLink":drive.webViewLink,"facturaArca.drivePendiente":false,facturaDrivePendiente:false,ultimoPdfFacturaId:drive.fileId,ultimoPdfFacturaUrl:drive.webViewLink});
      comp.driveFileId=drive.fileId;comp.driveFileName=drive.fileName;comp.driveWebViewLink=drive.webViewLink;comp.drivePendiente=false;
    }catch(e){driveError=e.message||String(e)}
    return res.json({ok:true,...comp,drive,driveError,drivePendiente:!drive,saldoNeto:nextFiscal.saldo});
  }catch(e){
