// TIZ V74.3 - asegura FC reales emitidas sin re-render destructivo
(function(){
'use strict';
const F={
...
function body(){const m=document.getElementById('cobr-modulo-v48');if(!m)return null;const cards=[...m.querySelectorAll('.card')];const card=cards.find(x=>/Seguimiento y compromisos|Cartera por cobrar/i.test(x.querySelector('.card-title')?.textContent||x.textContent||''));return card?.querySelector('tbody')||null;}
...
let scheduled=false;const obs=new MutationObserver(()=>{if(scheduled)return;scheduled=true;setTimeout(()=>{scheduled=false;ensure();},80)});setTimeout(()=>{const m=document.getElementById('cobr-modulo-v48');if(m)obs.observe(m,{childList:true,subtree:true});run();},100);
