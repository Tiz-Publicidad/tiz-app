...
function doPost(e) {
  try {
    ...
    const result = procesarPayload_(payload);
...
function doGet(e) {
  ...
  try {
    ...
    if (!raw) return jsonOrJsonp_({ ok:true, service:'TIZ Drive Sheets v5.6 JSONP' }, callback);
    const payload = JSON.parse(raw);
    const result = procesarPayload_(payload);
    return jsonOrJsonp_(result, callback);
  } catch (err) {
    return jsonOrJsonp_({ ok:false, error:String(err && err.message ? err.message : err) }, callback);
  }
}

function procesarPayload_(payload) {
  ...
  if (!obra.desc && !obra.ot) return { ok:false, error:'Faltan datos de obra' };
  ...
  const folderName = cleanName(`OT-${String(obra.ot || 'SIN_OT').padStart(6,'0')} ${obra.cliente || 'SIN_CLIENTE'} ${obra.desc || 'Obra'}`);
  const folder = getOrCreateFolder(parent, folderName);

  getOrCreateFolder(folder, 'Archivos Cliente');
  getOrCreateFolder(folder, 'Diseño');
  getOrCreateFolder(folder, 'Compras');
  getOrCreateFolder(folder, 'Producción');
  getOrCreateFolder(folder, 'Colocaciones');
  getOrCreateFolder(folder, 'Fotos finales');
  getOrCreateFolder(folder, 'Facturación');
