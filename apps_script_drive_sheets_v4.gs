...
function handleTizPayload_(payload) {
  ...
  writeValidacionCotizacionSheet_(ss, obra);
...
function writeValidacionCotizacionSheet_(ss, obra) {
  const sh = resetSheet_(ss, 'Validacion cotizacion');
  const v = obra.validacionCotizacion || {};
  const problemas = v.problemas || obra.problemasCotizacion || [];
  const rows = [
    ['Campo','Valor'],
    ['Estado validación', v.estado || ''],
    ['Score', v.score || ''],
    ['Cantidad problemas', problemas.length || 0],
    [],
    ['Nivel','Campo','Problema','Sugerencia']
  ];
  problemas.forEach(p => rows.push([p.level || '', p.campo || '', p.msg || String(p), p.sugerencia || '']));
  setRows_(sh, rows);
  sh.getRange(1,1,1,2).setFontWeight('bold').setBackground('#e8b84b');
  sh.getRange(6,1,1,4).setFontWeight('bold').setBackground('#e8b84b');
}
