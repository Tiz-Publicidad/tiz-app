...
(function () {
  ...
  function cli(o) {
    return (
      (window.DB?.clientes || []).find((c) => c.id === o.clienteId) ||
      (window.DB?.clientes || []).find(
        (c) => norm(c.nombre) === norm(o.cliente),
      ) ||
      {}
    );
  }
  function approved(o) {
    return num(
      o.finanzas?.total || o.neto || o.importe || o.infoPresupuesto?.importe,
    );
  }
  function comps(o) {
  ...
  function fiscal(o) {
    const p = approved(o),
      emit = comps(o).reduce(
        (s, c) => s + (c.familia === "credito" ? -num(c.neto) : num(c.neto)),
        0,
      );
    return { p, emit, saldo: Math.max(0, p - emit) };
  }
  function items(o, net) {
    ...
    const baseTotal =
        ...
        approved(o) ||
        net,
      ratio = baseTotal ? net / baseTotal : 1;
    return src.length
      ? src.map((x) => ({
          ...x,
  ...
  function suggestedType(cond) {
    return String(cond) === "1" || String(cond) === "6" ? 1 : 6;
  }
    ...
    return comps(o)
      .filter((c) => {
        const l =
          ...
          (/\bA\b/i.test(String(c.tipo || ""))
            ? "A"
            : /\bB\b/i.test(String(c.tipo || ""))
              ? "B"
              : "");
        return l === letter;
      })
      .map(
        (c) =>
          `<option value="${esc(JSON.stringify({ cbteTipo: c.cbteTipo, ptoVta: c.ptoVta, cbteNro: c.cbteNro, numeroCompleto: c.numeroCompleto, tipo: c.tipo }))}">${esc(c.tipo || "Comprobante")} · ${esc(c.numeroCompleto || "")} · ${money(c.neto)}</option>`,
      )
      .join("");
  }
  function ensureStyle() {
    if (document.getElementById("fv83-style")) return;
  ...
  window.abrirFacturacionIntegralV83 = function (id) {
    ...
    function drawItems() {
      ...
      itemsBox.innerHTML = its
        .map(
          (x) =>
            `<div><span>${esc(x.descripcion)}</span><span>${x.cantidad}</span><span>${money(x.unitario * x.cantidad)}</span></div>`,
        )
        .join("");
    }
    function syncAssoc() {
    ...
    pct.oninput = () => {
      net.value = ((f.p * num(pct.value)) / 100).toFixed(2);
      drawItems();
    };
    net.oninput = () => {
    ...
    $("#fv83-emit").onclick = async () => {
      const t = Number(typeEl.value),
        need = notes.has(t);
      if (need && !assoc.value)
        return alert("Seleccioná el comprobante original asociado.");
      const n = num(net.value);
      ...
      try {
        ...
        const wantEmail = $("#fv83-send")?.checked;
        ...
        alert(
          "No se pudo completar la emisión.\n\n" +
            (e.message || e) +
            "\n\nSi ARCA pudo haber autorizado, verificá antes de volver a intentar.",
        );
        b.disabled = false;
        b.textContent = old;
      }
