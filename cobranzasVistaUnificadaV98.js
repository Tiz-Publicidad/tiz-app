// TIZ V99 - Por cobrar unificado + FC 0001 preservada + buscador global de Cobranzas.
(function(){
'use strict';
...
const FC0001={cbteNro:1,numeroCompleto:'00009-00000001',driveFileId:'1OW0DBW9pH-QslFHdVJ--LHE2_q81s2jn',driveWebViewLink:'https://drive.google.com/file/d/1OW0DBW9pH-QslFHdVJ--LHE2_q81s2jn/view'};
...
function isFc0001(o,c){const ot=base(o?.ot);const n=Number(c?.cbteNro||0);const nr=String(c?.numeroCompleto||o?.nrfc||o?.finanzas?.saldo?.nroFactura||'');return ot==='4680'&&(n===1||/0000*1\b/.test(nr)||/00009[-_ ]?00000001/.test(nr))}
function latest(o){const c=[...comps(o)].sort((a,b)=>num(b?.cbteNro)-num(a?.cbteNro))[0]||o?.facturaArca||null;if(c)return c;if(base(o?.ot)==='4680'&&(o?.nrfc||o?.finanzas?.saldo?.nroFactura||o?.finanzas?.anticipo?.nroFactura))return {...FC0001};return null}
function hasInvoice(o){const f=o?.finanzas||{};return !!(o?.facturaArca?.cae||o?.nrfc||f?.anticipo?.facturado||f?.saldo?.facturado||f?.anticipo?.nroFactura||f?.saldo?.nroFactura||comps(o).length||base(o?.ot)==='4680')}
function emitted(o){const t=total(o),cs=comps(o);if(cs.length){const v=cs.reduce((s,c)=>s+num(c?.neto),0);if(v>0)return Math.min(t||v,v)}const f=o?.finanzas||{};let v=0;if(f?.anticipo?.facturado||f?.anticipo?.nroFactura)v+=num(f.anticipo.monto);if(f?.saldo?.facturado||f?.saldo?.nroFactura)v+=num(f.saldo.monto);if(v>0)return Math.min(t||v,v);if(o?.nrfc||base(o?.ot)==='4680')return t;return 0}
...
function drive(o){const c=latest(o);if(isFc0001(o,c))return {ok:true,href:FC0001.driveWebViewLink};if(!c)return {ok:false,href:''};const fileId=c.driveFileId||c.fileId||'';const href=c.driveWebViewLink||c.webViewLink||(fileId?`https://drive.google.com/file/d/${fileId}/view`:'');return {ok:!!(fileId||href),href}}
...
function pdf(o){if(!hasInvoice(o))return '<span style="color:var(--text3)">—</span>';const d=drive(o);if(d.ok)return d.href?`<a href="${esc(d.href)}" target="_blank" rel="noopener" class="badge badge-green" style="text-decoration:none">En Drive</a>`:'<span class="badge badge-green">En Drive</span>';const c=latest(o);if(!c?.cae)return '<span class="badge badge-gray">Factura histórica</span>';return `<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span class="badge badge-amber">PDF pendiente</span><button type="button" class="btn btn-ghost btn-sm tiz-v98-archive" data-n="${num(c.cbteNro)}">Archivar PDF</button></div>`}
