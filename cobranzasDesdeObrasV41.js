...
(function(){
  ...
  function renderForecast(obras){
    ...
    const weekOptions='<option value="all">Todas las semanas</option>'+list.map((w,i)=>`<option value="${weekKey(w.start)}" ${selected===weekKey(w.start)?'selected':''}>${i===0?'Esta semana':'Semana del '+shortDate(w.start)}</option>`).join('');
    box.innerHTML=`<div class="card" style="margin-top:0"><div class="card-header"><span class="card-title">Previsión de cobranzas por semana</span><select class="quick-estado" onchange="cambiarSemanaCobranzasV43(this.value)">${weekOptions}</select></div><div class="card-body"><div style="display:grid;grid-template-columns:repeat(8,minmax(95px,1fr));gap:8px;overflow-x:auto;padding-bottom:4px">${list.map((w,i)=>`<div onclick="cambiarSemanaCobranzasV43('${weekKey(w.start)}')" style="cursor:pointer;min-width:95px;background:var(--surface2);border:1px solid ${selected===weekKey(w.start)||selected==='all'&&i===0?'rgba(232,184,75,.5)':'var(--border)'};border-radius:9px;padding:10px"><div style="font-size:10px;color:${i===0?'var(--accent)':'var(--text3)'}">${i===0?'ESTA SEMANA':shortDate(w.start)}</div><div style="font-weight:600;margin:7px 0 5px">${MONEY.format(w.total)}</div><div style="height:4px;background:var(--border);border-radius:4px"><div style="height:100%;width:${Math.round(w.total/max*100)}%;background:var(--green);border-radius:4px"></div></div><div style="font-size:10px;color:var(--text3);margin-top:5px">${w.count} cobro${w.count===1?'':'s'}</div></div>`).join('')}</div></div></div>`+
