...
(function(){
  ...
  function setupPage(){
    ...
    const tabsValidas=['dashboard','facturar','cobrar','gestiones','retenciones','historico','alertas','configuracion'];
    if(!tabsValidas.includes(window.cobTab))window.cobTab='dashboard';
  ...
  function manageButton(o){
    ...
    const facturar=!facturada?`<button class="btn btn-primary btn-sm" onclick="abrirFacturacionGeneralV63('${o.id}')"><i class="ti ti-receipt"></i> Facturar</button>`:'';
    return `<div style="display:flex;gap:8px;align-items:center;justify-content:flex-end;flex-wrap:wrap">${estado}<button class="btn btn-ghost btn-sm" onclick="editarCobranzaObraV41('${o.id}')">Gestionar</button>${facturar}${enviar}</div>`;
  }
  ...
  function renderModule(obras){
    const permitidas=['dashboard','facturar','cobrar','gestiones','retenciones','historico','alertas','configuracion'];
    if(!permitidas.includes(window.cobTab))window.cobTab='dashboard';
    const module=document.getElementById('cobr-modulo-v48'),tab=window.cobTab;if(!module)return;
  ...
  window.abrirFacturacionGeneralV63=function(obraId){
    ...
    const cliente=(window.DB?.clientes||[]).find(c=>c.id===obra.clienteId||norm(c.nombre)===norm(obra.cliente))||{};
    const items=(Array.isArray(obra.itemsCotizados)?obra.itemsCotizados:[]).map(i=>({descripcion:String(i.descripcion||i.desc||'').trim(),cantidad:num(i.cantidad||i.cant||1),unitario:num(i.unitario??i.precio)})).filter(i=>i.descripcion&&i.cantidad>0&&i.unitario>0);
    const f=finances(obra),neto=num(f.total||obra.neto),cuit=String(cliente.cuit||obra.clienteCuit||obra.cuit||'').replace(/\D/g,'');
    document.getElementById('modal-fact-general-v63')?.remove();const root=document.createElement('div');root.id='modal-fact-general-v63';root.className='modal-overlay open';
    root.innerHTML=`<div class="modal" style="max-width:820px"><div class="modal-title">Facturar · OT ${esc(baseOt(obra.ot))} · ${esc(obra.cliente||'')}</div><div style="padding:10px 12px;border:1px solid var(--amber);border-radius:8px;margin-bottom:14px;font-size:12px"><b>HOMOLOGACIÓN · SIN VALOR FISCAL.</b> Esta pantalla todavía no puede emitir en producción.</div><div class="form-grid"><div class="form-group"><label>CUIT receptor</label><input id="fg-cuit" value="${esc(cuit)}"></div><div class="form-group"><label>Condición frente al IVA</label><select id="fg-cond"><option value="1">IVA Responsable Inscripto</option><option value="4">IVA Sujeto Exento</option><option value="5">Consumidor Final</option><option value="6">Responsable Monotributo</option></select></div><div class="form-group"><label>Comprobante</label><select id="fg-tipo"><option value="1">Factura A</option><option value="6">Factura B</option><option value="201">Factura de Crédito Electrónica A</option><option value="206">Factura de Crédito Electrónica B</option></select></div><div class="form-group"><label>IVA</label><select id="fg-iva"><option value="21">21%</option><option value="10.5">10,5%</option><option value="5">5%</option><option value="2.5">2,5%</option><option value="27">27%</option><option value="0">0%</option><option value="exento">Exento</option></select></div><div class="form-group"><label>Importe neto aprobado</label><input id="fg-neto" type="number" step="0.01" value="${neto.toFixed(2)}"></div><div class="form-group"><label>Destino posterior</label><select id="fg-envio"><option value="ahora">Enviar por correo ahora</option><option value="despues">Enviar por correo después</option><option value="otro">No enviar por correo</option></select></div></div><div style="font-size:12px;margin:12px 0">${items.length?`${items.length} ítem(s) cotizado(s) · Total calculado: ${MONEY.format(items.reduce((a,i)=>a+i.cantidad*i.unitario,0))}`:'<b style="color:var(--red)">No hay ítems cotizados válidos.</b>'}</div><label style="display:flex;gap:8px;font-size:12px"><input id="fg-ok" type="checkbox"> Revisé CUIT, condición fiscal, comprobante, IVA, ítems e importe.</label><div class="modal-actions"><button class="btn btn-ghost" id="fg-cancel">Cancelar</button><button class="btn btn-primary" id="fg-test" disabled>PROBAR EN HOMOLOGACIÓN</button></div></div>`;document.body.appendChild(root);
    root.querySelector('#fg-cancel').onclick=()=>root.remove();const ok=root.querySelector('#fg-ok'),btn=root.querySelector('#fg-test');ok.onchange=()=>btn.disabled=!ok.checked;
    btn.onclick=async()=>{const ivaValue=root.querySelector('#fg-iva').value,docNro=root.querySelector('#fg-cuit').value.replace(/\D/g,''),netoElegido=num(root.querySelector('#fg-neto').value);if(!/^\d{11}$/.test(docNro))return alert('Revisá el CUIT receptor.');if(!items.length)return alert('La obra no tiene ítems válidos para facturar.');btn.disabled=true;btn.textContent='Consultando ARCA…';try{const [{getApp},{getAuth}]=await Promise.all([import('https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js'),import('https://www.gstatic.com/firebasejs/10.12.0/firebase-auth.js')]);const user=getAuth(getApp()).currentUser;if(!user)throw new Error('Sesión no iniciada');const token=await user.getIdToken();const response=await fetch('https://us-central1-tiz---app.cloudfunctions.net/arcaHomologacionEmitirPrueba',{method:'POST',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},body:JSON.stringify({docNro,neto:netoElegido,items,cbteTipo:num(root.querySelector('#fg-tipo').value),condicionIVAReceptorId:num(root.querySelector('#fg-cond').value),tratamientoIva:ivaValue==='exento'?'exento':'gravado',alicuota:ivaValue==='exento'?0:num(ivaValue)})});const data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data.error||'ARCA rechazó la prueba');alert(`HOMOLOGACIÓN APROBADA · SIN VALOR FISCAL\n\nComprobante ${data.cbteNro}\nCAE de prueba: ${data.cae}\nTotal: ${MONEY.format(data.total)}`);root.remove()}catch(e){console.error(e);alert('No se pudo homologar.\n\n'+(e.message||e));btn.disabled=false;btn.textContent='PROBAR EN HOMOLOGACIÓN'}};
  };
...
window.abrirEnvioFacturaEmailV61=function(obraId){
    ...
    const cliente=(window.DB?.clientes||[]).find(c=>c.id===obra.clienteId||norm(c.nombre)===norm(obra.cliente));
    const normalizarLista=valor=>(Array.isArray(valor)?valor:String(valor||'').split(/[;,\\n]+/)).map(x=>String(x).trim().toLowerCase()).filter(x=>/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(x));
    let correos=[...new Set([
      ...normalizarLista(cliente?.emailsFacturacion),
      ...normalizarLista(cliente?.email),
      ...normalizarLista(factura.emailUltimoDestinatarios),
      ...normalizarLista(factura.emailUltimoDestinatario),
      ...normalizarLista(factura.emailDestinatario),
      ...(baseOt(obra.ot)==='4680'?['facturacion@rootsagencia.com']:[])
    ])].slice(0,10);
    ...
    root.innerHTML=`<div class="modal" style="max-width:720px"><div class="modal-title">Enviar factura por correo</div><div style="padding:10px 12px;border:1px solid rgba(232,184,75,.5);background:rgba(232,184,75,.08);border-radius:8px;margin-bottom:14px;font-size:12px">Factura <b>${esc(numero)}</b> · OT <b>${esc(baseOt(obra.ot))}</b>. Elegí exactamente quiénes la recibirán.</div><div class="form-group"><label>Destinatarios guardados del cliente</label><div id="fc-email-list-v61" style="display:grid;gap:7px;margin:6px 0 10px"></div><div style="display:flex;gap:7px"><input id="fc-email-new-v61" type="email" placeholder="nuevo@cliente.com" style="flex:1"><button class="btn btn-ghost" id="fc-email-add-v61" type="button">Agregar</button></div><div style="font-size:11px;color:var(--text3);margin-top:5px">Hasta 10 correos. Agregar o quitar actualiza la lista habitual del cliente.</div></div><div class="form-group"><label>Asunto</label><input id="fc-email-asunto-v61" value="Factura ${esc(numero)} - ${esc(obra.cliente||'')}" maxlength="180"></div><label style="display:flex;gap:9px;align-items:flex-start;font-size:12px;margin-top:14px"><input id="fc-email-confirm-v61" type="checkbox" style="margin-top:2px"> Revisé los destinatarios seleccionados y deseo enviar ahora esta factura real.</label><div class="modal-actions"><button class="btn btn-ghost" id="fc-email-cancel-v61">ENVIAR MÁS TARDE</button><button class="btn btn-primary" id="fc-email-send-v61" disabled>ENVIAR AHORA</button></div></div>`;
    document.body.appendChild(root);
    const list=root.querySelector('#fc-email-list-v61'),send=root.querySelector('#fc-email-send-v61'),confirm=root.querySelector('#fc-email-confirm-v61');
    const persistir=async()=>{if(!cliente)return;await window.updateDoc_('clientes',cliente.id,{emailsFacturacion:[...correos],email:correos[0]||''});cliente.emailsFacturacion=[...correos];cliente.email=correos[0]||''};
    const renderLista=()=>{list.innerHTML=correos.length?correos.map((email,i)=>`<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--border);border-radius:7px"><input type="checkbox" data-email="${esc(email)}" ${seleccionados.has(email)?'checked':''}><span style="flex:1">${esc(email)}</span><button type="button" class="btn btn-ghost btn-sm" data-remove="${i}" title="Quitar correo"><i class="ti ti-trash"></i></button></div>`).join(''):'<div style="color:var(--text3);font-size:12px">Todavía no hay correos guardados.</div>';list.querySelectorAll('input[data-email]').forEach(x=>x.onchange=()=>{x.checked?seleccionados.add(x.dataset.email):seleccionados.delete(x.dataset.email)});list.querySelectorAll('[data-remove]').forEach(x=>x.onclick=async()=>{const email=correos[Number(x.dataset.remove)];correos=correos.filter(e=>e!==email);seleccionados.delete(email);renderLista();try{await persistir()}catch(e){console.error(e);alert('No se pudo actualizar la lista del cliente.')}})};
    renderLista();
    root.querySelector('#fc-email-add-v61').onclick=async()=>{const input=root.querySelector('#fc-email-new-v61'),email=input.value.trim().toLowerCase();if(!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(email))return alert('Ingresá un correo válido.');if(!correos.includes(email)&&correos.length>=10)return alert('Podés guardar hasta 10 correos.');if(!correos.includes(email))correos.push(email);seleccionados.add(email);input.value='';renderLista();try{await persistir();window.showToast?.('Correo agregado al cliente')}catch(e){console.error(e);alert('No se pudo guardar el correo.')}};
    root.querySelector('#fc-email-cancel-v61').onclick=()=>{root.remove();window.showToast?.('Factura pendiente de envío')};
    confirm.onchange=()=>{send.disabled=!confirm.checked};
    send.onclick=async()=>{
      const elegidos=correos.filter(email=>seleccionados.has(email));
      const asunto=root.querySelector('#fc-email-asunto-v61').value.trim();
      if(!elegidos.length)return alert('Tildá al menos un destinatario.');
      ...
      try{
        ...
        const destinatarios=elegidos.join(',');
  ...
  async function emitirFacturaRealOT4680V60(obra,root,button,abrirEnvioDespues){
    ...
    try{
      ...
      if(abrirEnvioDespues)setTimeout(()=>window.abrirEnvioFacturaEmailV61?.(obra.id),0);else window.showToast?.('Factura emitida · envío pendiente');
  ...
  window.editarCobranzaObraV41=function(id){
    ...
    const modoInicial=(f.anticipo.facturado||f.anticipo.nroFactura||num(f.anticipo.monto)>0)?'anticipo':'total';
    root.innerHTML=`<div class="modal" style="max-width:820px"><div class="modal-title">Gestión de cobranza · OT ${esc(baseOt(o.ot))} · ${esc(o.cliente)}</div><div class="form-grid"><div class="form-group"><label>Total de la obra</label><input id="fin-total" type="number" min="0" value="${num(f.total)}"></div><div class="form-group"><label>Plazo habitual de pago</label><select id="fin-dias-pago"><option value="0">Sin plazo definido</option>${[7,15,30,45,60,90].map(d=>`<option value="${d}" ${dias===d?'selected':''}>${d} días</option>`).join('')}</select></div><div class="form-section">Tipo de factura</div><div class="form-group full"><div style="display:flex;gap:18px;flex-wrap:wrap;padding:10px 12px;border:1px solid var(--border);border-radius:8px"><label style="display:flex;align-items:center;gap:7px"><input type="radio" name="fin-tipo-factura" value="total" ${modoInicial==='total'?'checked':''}> Factura total</label><label style="display:flex;align-items:center;gap:7px"><input type="radio" name="fin-tipo-factura" value="anticipo" ${modoInicial==='anticipo'?'checked':''}> Factura de anticipo</label></div><small style="color:var(--text3)">El porcentaje sólo se solicita cuando elegís anticipo.</small></div>${partFields('fin-ant','Factura de anticipo',f.anticipo,f.total)}${partFields('fin-sal','Factura total / saldo',f.saldo,f.total)}<details class="form-group full" style="border:1px solid var(--border);border-radius:8px;padding:10px 12px"><summary style="cursor:pointer;font-weight:600">Retenciones y notas</summary><div class="form-grid" style="margin-top:12px">${moneyInput('fin-suss','SUSS',f.retenciones.suss)}${moneyInput('fin-iibb','Ingresos Brutos',f.retenciones.iibb)}${moneyInput('fin-gan','Ganancias',f.retenciones.ganancias)}${moneyInput('fin-iva','IVA',f.retenciones.iva)}${moneyInput('fin-otras','Otras',f.retenciones.otras)}<div class="form-group full"><label>Notas privadas de cobranzas</label><textarea id="fin-notas">${esc(f.notas)}</textarea></div></div></details></div><div class="modal-actions"><button class="btn btn-ghost" id="fin-cancel">Cancelar</button><button class="btn btn-ghost" id="fin-save">Guardar gestión</button><button class="btn btn-primary" id="fin-invoice">Facturar</button></div></div>`;
    document.body.appendChild(root);root.querySelector('#fin-cancel').onclick=()=>root.remove();root.querySelector('#fin-save').onclick=()=>guardarFinanzas(id,root);root.querySelector('#fin-invoice').onclick=()=>guardarFinanzas(id,root,true);root.querySelector('#fin-dias-pago').onchange=()=>calcularFechasPrevistasV44(true);const aplicarModo=()=>{const modo=root.querySelector('[name="fin-tipo-factura"]:checked')?.value||'total',ant=root.querySelector('#fin-ant-section'),sal=root.querySelector('#fin-sal-section');if(ant)ant.style.display=modo==='anticipo'?'contents':'none';if(sal)sal.style.display=modo==='total'?'contents':'none';if(modo==='total'){const pct=root.querySelector('#fin-sal-pct'),monto=root.querySelector('#fin-sal-monto');if(pct)pct.value=100;if(monto&&!num(monto.value))monto.value=num(root.querySelector('#fin-total')?.value)}};root.querySelectorAll('[name="fin-tipo-factura"]').forEach(x=>x.onchange=aplicarModo);aplicarModo();['fin-ant','fin-sal'].forEach(p=>{const pct=document.getElementById(p+'-pct'),amount=document.getElementById(p+'-monto');pct.oninput=()=>{amount.value=Math.round(num(val('fin-total'))*num(pct.value))/100};amount.oninput=()=>{pct.value=num(val('fin-total'))?Math.round(num(amount.value)/num(val('fin-total'))*10000)/100:0};document.getElementById(p+'-ff').onchange=()=>calcularFechasPrevistasV44(false)});
  };
  ...
  async function guardarFinanzas(id,root,abrirFactura=false){
  ...
  function instalar(){
    ...
    if(!window.goTo.__cobranzasV41){const baseGo=window.goTo;const secured=function(page){if(page==='cobranzas'&&!window.currentUser?.isAdmin){window.showToast?.('Cobranzas es un módulo privado de administración');return}const result=baseGo.apply(this,arguments);if(page==='cobranzas'){if(!['dashboard','facturar','cobrar','gestiones','retenciones','historico','alertas','configuracion'].includes(window.cobTab))window.cobTab='dashboard';requestAnimationFrame(()=>{setupPage();window.renderCobranzas?.()})}return result};secured.__cobranzasV41=true;window.goTo=secured;}
    setupPage();
