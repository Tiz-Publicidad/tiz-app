...
(function(){
  ...
  function renderModule(obras){
    ...
    }else if(tab==='cobrar'){const items=obras.filter(o=>totals(o).pendiente>0&&(finances(o).anticipo.facturado||finances(o).saldo.facturado||finances(o).anticipo.nroFactura||finances(o).saldo.nroFactura)).sort((a,b)=>(parseIsoDate(due(a))||new Date(9999,1,1))-(parseIsoDate(due(b))||new Date(9999,1,1)));module.innerHTML=tableCard('Cartera por cobrar',['OT','Cliente / obra','Facturación','Saldo','Cobro previsto','Estado','Acción'],operationalRows(items),'No hay facturas pendientes de cobro.');
    }else if(tab==='gestiones'){const items=obras.filter(o=>totals(o).pendiente>0&&(!due(o)||finances(o).notas)).sort((a,b)=>Number(!due(a))-Number(!due(b)));module.innerHTML=tableCard('Seguimiento y compromisos',['OT','Cliente / obra','Facturación','Saldo','Fecha comprometida','Estado','Acción'],operationalRows(items),'No hay gestiones abiertas.');
