...
window.abrirEnvioFacturaEmailV61=function(obraId){
  ...
  window.prepararFacturaArcaV63=function(obraId){
    ...
    root.querySelector('#arca-emit-v63').onclick=async()=>{
      const button=root.querySelector('#arca-emit-v63'),tipoParte=root.querySelector('[name="arca-parte-v63"]:checked')?.value||modoInicial,porcentaje=tipoParte==='anticipo'?num(root.querySelector('#arca-pct-v63').value):100,condicionIvaId=num(root.querySelector('#arca-condicion-v63').value),tipoComprobante=root.querySelector('#arca-tipo-v63').value,fechaVtoPago=root.querySelector('#arca-vto-v63')?.value||'',cuitElegido=root.querySelector('#arca-cuit-v63').value.replace(/\D/g,'');
      if(!/^\d{11}$/.test(cuitElegido))return alert('Ingresá un CUIT válido de 11 dígitos.');
      if(tipoParte==='anticipo'&&(porcentaje<=0||porcentaje>=100))return alert('El anticipo debe ser mayor a 0% y menor a 100%.');if(tipoComprobante==='A'&&condicionIvaId!==1)return alert('Factura A requiere cliente Responsable Inscripto.');if(tipoComprobante==='B'&&condicionIvaId===1)return alert('Para un Responsable Inscripto corresponde Factura A.');if(tipoComprobante==='FCEA'&&!fechaVtoPago)return alert('Indicá el vencimiento de pago de la FCE.');
