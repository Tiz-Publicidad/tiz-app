...
(() => {
  ...
  function drawTab() {
    ...
    const log=o.entregaLogistica||{}, info=o.infoPresupuesto||data.infoPresupuesto||{}, modalidades={a_definir:'A definir',retiro_fabrica:'Retira en fábrica',envio:'Envío a domicilio',colocacion:'Con colocación'};
    const items=(info.items||o.itemsTecnicos||[]).map(x=>x.articulo||x.descripcion||x.desc).filter(Boolean), aclaraciones=(info.aclaraciones||[]).filter(Boolean);
    const compartida=`<div class="v31-panel" style="margin-bottom:10px;border-left:3px solid ${sector==='Colocaciones'?'var(--teal)':'var(--blue)'}"><b>Información recibida desde Presupuestos</b><div class="v31-form" style="margin-top:8px"><div><span class="v31-sub">Modalidad</span><br><b>${esc(modalidades[log.tipo||info.modalidadEntrega]||'A definir')}</b></div><div><span class="v31-sub">Fecha prevista</span><br><b>${esc(log.fecha||info.fechaEntrega||'Sin definir')}</b></div>${info.plazoEstimado?`<div><span class="v31-sub">Plazo informado</span><br><b>${esc(info.plazoEstimado)}</b></div>`:''}${log.domicilio||info.domicilio?`<div><span class="v31-sub">Domicilio</span><br><b>${esc(log.domicilio||info.domicilio)}</b></div>`:''}${log.contacto||info.contacto?`<div><span class="v31-sub">Contacto</span><br><b>${esc(log.contacto||info.contacto)}</b></div>`:''}${log.retira||info.retira?`<div><span class="v31-sub">Retira</span><br><b>${esc(log.retira||info.retira)}</b></div>`:''}${items.length?`<div class="v31-field full"><span class="v31-sub">Ítems / materiales</span><div>${items.map(esc).join(' · ')}</div></div>`:''}${aclaraciones.length?`<div class="v31-field full"><span class="v31-sub">Aclaraciones del presupuesto</span><div>${aclaraciones.map(esc).join('<br>')}</div></div>`:''}${log.detalle||info.indicacionesEntrega?`<div class="v31-field full"><span class="v31-sub">Indicaciones</span><div>${esc(log.detalle||info.indicacionesEntrega)}</div></div>`:''}</div></div>`;
  ...
  window.syncProductionOtDocumentV315=async function(o){
    ...
    const result=await callWebhook({action:'syncSectorDocument',obra:{firestoreId:o.id||o.firestoreId,ot:o.ot,cliente:o.cliente,desc:o.desc,estado:o.estado,driveFolderId:o.driveFolderId||'',driveFolderUrl:o.driveFolderUrl||'',sector:'Producción',data:{...productionSheetPayload(data),entregaLogistica:o.entregaLogistica||{},infoPresupuesto:o.infoPresupuesto||{}}}});
