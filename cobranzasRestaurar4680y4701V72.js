...
(function(){
  ...
  const FACTURAS={
    '4680':{numero:'00009-00000001',fecha:'04/09/2026',enviada:true,cliente:'Actitud Argentina'},
    '4701':{numero:'00009-00000002',fecha:'07/09/2026',enviada:false,cliente:'River Plate'}
  };
  function saldo(o){
  ...
  function insertar(){
    ...
    for(const ot of ['4701','4680']){
      ...
      const o=(window.DB?.obras||[]).find(x=>baseOt(x.ot)===ot);if(!o)continue;
      const d=FACTURAS[ot],p=saldo(o)||num(o?.finanzas?.total||o?.neto);
      const tr=document.createElement('tr');tr.dataset.tizV72=ot;
      tr.innerHTML=`<td class="strong">${ot}</td><td><b>${esc(o.cliente||d.cliente)}</b><br><span style="color:var(--text3)">${esc(o.desc||'')}</span></td><td>Saldo: FC ${d.numero} · ${d.fecha}</td><td>${money(p)}</td><td>—</td><td><select class="quick-estado" style="min-width:170px"><option>${d.enviada?'Factura enviada':'Facturada - falta enviar'}</option></select></td><td><div style="display:flex;gap:8px;align-items:center;justify-content:flex-end;flex-wrap:wrap"><span title="${d.enviada?'Factura enviada por correo':'Factura pendiente de envío por correo'}" style="width:11px;height:11px;border-radius:50%;display:inline-block;background:${d.enviada?'#22a06b':'#e8b84b'}"></span><button class="btn btn-ghost btn-sm" onclick="editarCobranzaObraV41('${o.id}')">Gestionar</button>${ot==='4701'?`<button class="btn btn-ghost btn-sm" onclick="abrirEnvioFacturaEmailV61('${o.id}')"><i class="ti ti-mail-forward"></i> Enviar FC</button>`:`<button class="btn btn-ghost btn-sm" onclick="abrirEnvioFacturaEmailV61('${o.id}')"><i class="ti ti-mail-forward"></i> Reenviar FC</button>`}</div></td>`;
      body.prepend(tr);
    }
  }
